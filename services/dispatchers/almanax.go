...
func (service *Impl) dispatchAlmanax(correlationID string, almanax *amqp.NewsRSSMessage, locale amqp.Language) {
	...
	webhooks, err := service.webhookService.GetAlmanaxWebhooks(locale)
	if err != nil {
		log.Error().Err(err).
...
func (service *Impl) updateAlmanaxWebhooks(webhooks []entities.WebhookAlmanax) {
	err := service.webhookService.UpdateAlmanaxWebhooks(webhooks)
	if err != nil {
		log.Error().Err(err).
...
func (service *Impl) deleteAlmanaxWebhooks(webhooks []entities.WebhookAlmanax) {
	err := service.webhookService.DeleteAlmanaxWebhooks(webhooks)
	if err != nil {
		log.Error().Err(err).
